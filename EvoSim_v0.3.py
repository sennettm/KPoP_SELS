...
def calc_G_F(tmp_seq, lookup_table, res_nums, AA_miya):
    ...
    for res_pair in lookup_table:
        ...
        #print(seq_res_one, seq_res_two)
        #calc CP between two residues
        miya_id_one=AA_miya_dct.get(seq_res_one)
        miya_id_two=AA_miya_dct.get(seq_res_two)
        #print(miya_id_one, miya_id_two)
        fold_GNS.append(miyazawa[miya_id_one][miya_id_two])
        #fold_GNS.append(miyazawa_5_upp[miya_id_one][miya_id_two])

    #print(fold_GNS[-1], res_pair, aa_one, aa_two, seq_res)
...
def calc_Pfix(wt_aa_seq, var_aa_seq, lookup_table, res_nums, rc_lookup_tables, rc_res_dct, N_eff, m, fit):
    ...
    if fit == 1:
        ...
        #my method
        #dG_i=G_i+np.log(seq_len**3.4)+(std_G_i_rc**2-avg_G_i_rc)/2
        #goldstein
        #kT=(1.380649*(10**-23))*293
        RT=0.593
        ...
        #my method
        #dG_v=G_v+np.log(seq_len**3.4)+(std_G_v_rc**2-avg_G_v_rc)/2
        #goldstein
        #kbT=(1.380649*(10**23))*293
        ...
        #wi=np.longdouble(1)/(np.longdouble(1)+np.exp(dG_i))
        #wv=np.longdouble(1)/(np.longdouble(1)+np.exp(dG_v))
        ...
        for i in range(len(x)):
            ...
            if x[i].taxon.label not in aln:
                ...
                while m < exp_mut:
                    ...
                    dice=rd.uniform(0,1)
                    #print(dG_i, dG_v, ddG, G_i, G_v, avg_G_i_rc, std_G_i_rc, avg_G_v_rc, std_G_v_rc)
